...
let requests: BloodRequest[] = [
  ...
  { id: '105', requestorId: '3', patientName: 'Varghese Mathew', bloodGroup: BloodGroup.OPositive, units: 4, hospital: 'Holy Cross Hospital', locality: 'Kottiyam', urgency: 'High', status: 'Matched', createdAt: new Date(Date.now() - 86400000), matchedDonorId: '2'},
];
...
const createRequest = async (requestData: Omit<BloodRequest, 'id' | 'createdAt' | 'status' | 'matchedDonorId'>): Promise<BloodRequest> => {
  await delay(1000);
...
const deleteRequest = async (requestId: string): Promise<void> => {
    ...
    if (requests.length === initialLength) {
        // Optional: throw an error if the request to be deleted was not found
        console.warn(`Request with ID ${requestId} not found for deletion.`);
    }
};
...
const findMatches = async (requestId: string): Promise<User[]> => {
  ...
  if(!request) return [];
  await delay(700);
  const matchingUsers = users.find(user =>
      (user.role === UserRole.Volunteer || user.role === UserRole.Both) &&
  ...
  return matchingUsers ? [matchingUsers].map(({password, ...user}) => user) : [];
};

const updateUser = async (userId: string, updateData: Partial<User & { currentPassword?: string, newPassword?: string }>): Promise<User> => {
    await delay(600);
    ...
    const allowedUpdates: (keyof User)[] = ['name', 'email', 'phone', 'locality', 'role'];

    for (const key in updateData) {
        if (allowedUpdates.includes(key as keyof User)) {
            if (key === 'email' && updateData.email !== user.email) {
                // Check if new email already exists
                if (users.some(u => u.email === updateData.email && u.id !== userId)) {
